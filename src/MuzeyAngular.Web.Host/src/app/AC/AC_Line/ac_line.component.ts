...
@Component({
    templateUrl: './ac_line.component.html',
})
export class ACLineComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        { label: '线体分组', name: 'LineGroup' },
        { label: '线体编码', name: 'LineCode' },
        { label: '线体首位号', name: 'LineName' },
        { label: '线体描述', name: 'LineDesc' },
        { label: '线体中文名称', name: 'LineFullName' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACLine.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACLineEditComponent, { data: { workShop: this.reqModel.workShop } });	
        } else {
            editDialog = this._dialog.open(ACLineEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACLine.Delete";
...
@Component({
    templateUrl: './ac_line_edit.component.html'
})
export class ACLineEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACLineEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACLine.GetData";
