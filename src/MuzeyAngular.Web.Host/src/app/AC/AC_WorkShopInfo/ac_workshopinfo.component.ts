...
@Component({
    templateUrl: './ac_workshopinfo.component.html',
})
export class ACWorkShopInfoComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    totalCount: number;
    cols: ColModel[] = [
        { label: '车间编码', name: 'WorkShopCode' },
        { label: '车间名', name: 'WorkShopName' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACWorkShopInfo.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACWorkShopInfoEditComponent, { data: { workShop: this.reqModel.workShop }});
        } else {
            editDialog = this._dialog.open(ACWorkShopInfoEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACWorkShopInfo.Delete";
...
@Component({
    templateUrl: './ac_workshopinfo_edit.component.html'
})
export class ACWorkShopInfoEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACWorkShopInfoEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACWorkShopInfo.GetData";
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACWorkShopInfo.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
