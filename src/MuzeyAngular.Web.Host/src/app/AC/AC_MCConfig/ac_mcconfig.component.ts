...
@Component({
    templateUrl: './ac_mcconfig.component.html',
})
export class ACMCConfigComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        ...
        { label: '灯号', name: 'TagID' },
        { label: '状态', name: 'State' },
        { label: 'PLC给MQ物料呼叫的OPC地址', name: 'PlcToMqAddressId' },
        { label: 'MQ给PLC物料应答的OPC地址', name: 'MqToPlcAnswerAddressId' },
        { label: 'MQ给PLC物料送达的OPC地址', name: 'MqToPlcArriveAddressId' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACMCConfig.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACMCConfigEditComponent, { data: { workShop: this.reqModel.workShop } });
        } else {
            editDialog = this._dialog.open(ACMCConfigEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACMCConfig.Delete";
...
@Component({
    templateUrl: './ac_mcconfig_edit.component.html'
})
export class ACMCConfigEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACMCConfigEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACMCConfig.GetData";
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACMCConfig.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
