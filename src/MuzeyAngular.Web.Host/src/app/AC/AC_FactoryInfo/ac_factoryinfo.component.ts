...
@Component({
    templateUrl: './ac_factoryinfo.component.html',
})
export class ACFactoryInfoComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        { label: '工厂代码', name: 'FactoryCode' },
        { label: '工厂名称', name: 'FactoryName' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACFactoryInfo.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACFactoryInfoEditComponent, { data: { workShop: this.reqModel.workShop }});
        } else {
            editDialog = this._dialog.open(ACFactoryInfoEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACFactoryInfo.Delete";
...
@Component({
    templateUrl: './ac_factoryinfo_edit.component.html'
})
export class ACFactoryInfoEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACFactoryInfoEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACFactoryInfo.GetData";
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACFactoryInfo.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
