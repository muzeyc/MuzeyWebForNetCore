...
@Component({
    templateUrl: './ac_mailbaseconfig.component.html',
})
export class ACMailBaseConfigComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    totalCount: number;
    cols: ColModel[] = [
        { label: '发件人账号', name: 'Adresser' },
        { label: '发件人账号密码', name: 'AdresserPwd' },
        { label: '预警时间(分钟)', name: 'PrecautiousT' },
        { label: '推送时间间隔(分钟)', name: 'PushedInterval' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACMailBaseConfig.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACMailBaseConfigEditComponent, { data: { workShop: this.reqModel.workShop }});
        } else {
            editDialog = this._dialog.open(ACMailBaseConfigEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACMailBaseConfig.Delete";
...
@Component({
    templateUrl: './ac_mailbaseconfig_edit.component.html'
})
export class ACMailBaseConfigEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACMailBaseConfigEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACMailBaseConfig.GetData";
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACMailBaseConfig.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
