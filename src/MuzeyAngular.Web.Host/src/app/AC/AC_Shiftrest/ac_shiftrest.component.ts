...
@Component({
    templateUrl: './ac_shiftrest.component.html',
})
export class ACShiftrestComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        ...
        { label: '生产时间', name: 'workDay' },
        { label: '班次名', name: 'ShiftName' },
        { label: '休息类别', name: 'RestName' },
        //{ label: '休息开始时间', name: 'BeginTime' },
        //{ label: '休息结束时间', name: 'EndTime' },
        { label: '休息开始时间', name: 'sT' },
        { label: '休息结束时间', name: 'eT' },
        //{ label: '是否跨天', name: 'CrossDay' },
        { label: '是否跨天', name: 'crossDayName' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACShiftrest.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACShiftrestEditComponent, { data: { workShop: this.reqModel.workShop } });	
        } else {
            editDialog = this._dialog.open(ACShiftrestEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACShiftrest.Delete";
...
@Component({
    templateUrl: './ac_shiftrest_edit.component.html'
})
export class ACShiftrestEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACShiftrestEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACShiftrest.GetData";
        if (!isNullOrUndefined(this._data.id)) {
            this.reqModel.saveData = {};
            this.reqModel.saveData.ID = this._data.id;
            serverReq.datas.push(this.reqModel);
            this._service.req(serverReq).subscribe(result => {
                result.datas[0].CrossDay = result.datas[0].CrossDay.toString(); 
                this.reqModel.saveData = result.datas[0];
                this.reqModel.saveData.BeginTime = this.reqModel.saveData.BeginTime.split('T')[1];
                this.reqModel.saveData.EndTime = this.reqModel.saveData.EndTime.split('T')[1];
            });
        }
    }
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACShiftrest.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
