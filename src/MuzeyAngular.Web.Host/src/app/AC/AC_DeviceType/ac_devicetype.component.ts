...
@Component({
    templateUrl: './ac_devicetype.component.html',
})
export class ACDeviceTypeComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        ...
        { label: '设备类型名称', name: 'DeviceTypeName' },
        { label: '设备类型描述', name: 'DeviceTypeDesc' },
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACDeviceType.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACDeviceTypeEditComponent, { data: { workShop: this.reqModel.workShop } });
        } else {
            editDialog = this._dialog.open(ACDeviceTypeEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACDeviceType.Delete";
...
@Component({
    templateUrl: './ac_devicetype_edit.component.html'
})
export class ACDeviceTypeEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACDeviceTypeEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACDeviceType.GetData";
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACDeviceType.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
