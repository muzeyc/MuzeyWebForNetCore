...
@Component({
    templateUrl: './ac_semouldinfo.component.html',
})
export class ACSEMouldInfoComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        { label: '模具编号', name: 'MouldCode' },
        { label: '项目名称', name: 'ProjectName' },
        { label: '左零件编号', name: 'LeftPartCode' },
        { label: '左零件名称', name: 'LeftPartName' },
        { label: '右零件编号', name: 'RightPartCode' },
        { label: '右零件名称', name: 'RightPartName' }
    ];
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACSEMouldInfo.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACSEMouldInfoEditComponent, { data: { workShop: this.reqModel.workShop }});
        } else {
            editDialog = this._dialog.open(ACSEMouldInfoEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACSEMouldInfo.Delete";
                    if (!isNullOrUndefined(this.mtc.itemObj.MouldCode)) {
                        this.reqModel.saveData = {};
                        this.reqModel.saveData.MouldCode = this.mtc.itemObj.MouldCode;
                        serverReq.datas.push(this.reqModel);
...
@Component({
    templateUrl: './ac_semouldinfo_edit.component.html'
})
export class ACSEMouldInfoEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACSEMouldInfoEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACSEMouldInfo.GetData";
        if (!isNullOrUndefined(this._data.id)) {
            this.reqModel.saveData = {};
            this.reqModel.saveData.MouldCode = this._data.id;
            this.reqModel.op = 'update';
            serverReq.datas.push(this.reqModel);
            this._service.req(serverReq).subscribe(result => {
                this.reqModel.saveData = result.datas[0];
            });
        } else {
            this.reqModel.op = 'add';
        }
    }
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACSEMouldInfo.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
