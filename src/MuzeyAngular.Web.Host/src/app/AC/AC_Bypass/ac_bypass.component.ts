...
@Component({
    templateUrl: './ac_bypass.component.html',
})
export class ACBypassComponent extends AppComponentBase implements OnInit {
    @ViewChild('mtc')
    ...
    cols: ColModel[] = [
        ...
        { label: 'VIN', name: 'VIN' },
        { label: 'BYPASS状态', name: 'BypassStatus' },
        { label: 'BYPASS发生时间', name: 'BypassStartTime' },
        { label: 'BYPASS结束时间', name: 'BypassEndTime' },
    ...
    getDataPage(offset: number): void {
        ...
        serverReq.datas = [];
        serverReq.action = "ACBypass.GetDatas";
    ...
    edit(id?: string): void {
        ...
        if (isNullOrUndefined(id)) {
            editDialog = this._dialog.open(ACBypassEditComponent, { data: { workShop: this.reqModel.workShop }});
        } else {
            editDialog = this._dialog.open(ACBypassEditComponent, {
        ...
        abp.message.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let serverReq = new MuzeyReqModelOfObject();
                    serverReq.datas = [];
                    serverReq.action = "ACBypass.Delete";
...
@Component({
    templateUrl: './ac_bypass_edit.component.html'
})
export class ACBypassEditComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        injector: Injector,
        private _service: MuzeyServiceProxy,
        private _dialogRef: MatDialogRef<ACBypassEditComponent>,
    ...
    ngOnInit(): void {
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACBypass.GetData";
    ...
    save(): void {
        ...
        let serverReq = new MuzeyReqModelOfObject();
        serverReq.datas = [];
        serverReq.action = "ACBypass.Save";
        serverReq.datas.push(this.reqModel);
        this._service.req(serverReq)
            .pipe(
                finalize(() => {
