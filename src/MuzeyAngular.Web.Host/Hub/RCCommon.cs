...
namespace MuzeyRC
{
    public class RCCommon
    {
        ...
        public static string RCUpdateRoadIn(string Area, string Road, string vin)
        {
            try
            {
                var dto = SqlHelp.Query(string.Format("◎ABP_Base◎SELECT TOP 1 * FROM RC_Cache WHERE Area='{0}' and (vin is null or vin='') AND Road LIKE '%{1}'", Area, Road)).Tables[0].DataTableToList<RC_CacheDto>()[0];
                dto.VIN = vin;
                if (!string.IsNullOrEmpty(vin))
                {
                    var maxSeq = SqlHelp.Query(string.Format("◎ABP_Base◎SELECT TOP 1 ISNULL(max(Seq), 0) as seq from RC_Cache where Area = '{0}'", Area)).Tables[0].Rows[0][0].ToInt();
                    dto.Seq = maxSeq + 1;
                }
                ...
                {
                    dto.State = "1";
                    ThreadHelp.log.Debug("VIN->" + vin + "为预冻结车辆入道后直接冻结");
                    //删除预冻结
                    opDal.DeleteDtoList(dDtos);
                    ThreadHelp.log.Debug("VIN->" + vin + "删除该预冻结记录");
                }
                //确认是否为冻结车返回重进
                ...
                {
                    dto.State = "1";
                    ThreadHelp.log.Debug("VIN->" + vin + "为冻结车优先进冻结车道");
                    // 删除冻结记录
                    opDal.DeleteDtoList(dcDtos);
                    ThreadHelp.log.Debug("VIN->" + vin + "删除该冻结记录");
                }
                dal.UpdateDtoToPart(dto);
                ThreadHelp.log.Debug("队列更新成功->区域->" + Area + "->车道->" + Road + "->VIN->" + vin);
                return "0";
            }
            catch(Exception e)
            {
                ThreadHelp.log.Debug("队列更新失败->区域->" + Area + "->车道->" + Road + "->VIN->" + vin);
                ThreadHelp.log.Debug(e.Message + "->" + e.StackTrace);
        ...
        public static string RCUpdateRoadOut(string Area, string Road, string vin)
        {
            try
            {
                ...
                if (dtos[0].State == "1")
                {
                    ...
                    ThreadHelp.log.Debug("插入冻结车记录->" + Area + "->车道->" + Road + "->VIN->" + vin);
                }
